...
function New-ConstantsFile ($filePath)
{
	...
	Add-Content -Value "platform=$TestPlatform" -Path $filePath
...
function Main()
{
	...
	try
	{
		...
		RemoteCopy -uploadTo $hs1VIP -port $hs1vm1sshport -files "$constantsFile,.\$LogDir\StartFioTest.sh,.\$LogDir\ParseFioTestLogs.sh" -username $user -password $password -upload
